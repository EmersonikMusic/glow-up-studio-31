    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
      {answers.map((answer, index) => {
        ...
        let stateStyle: React.CSSProperties = {
          background: color.bg,
          boxShadow: "0 4px 16px hsl(240 45% 10% / 0.4)",
        };
        let icon = null;
        ...
        if (hasAnswered) {
          if (isCorrect) {
            stateStyle = {
              ...
              boxShadow: "0 0 0 2px hsl(140 65% 60%), 0 0 20px hsl(140 65% 50% / 0.3)",
            };
            icon = <CheckCircle2 className="w-5 h-5 text-white flex-shrink-0" />;
          } else if (isSelected && !isCorrect) {
            stateStyle = {
              background: "hsl(var(--game-wrong))",
              boxShadow: "0 0 0 2px hsl(0 70% 65%)",
            };
        ...
        return (
          <button
            ...
            className={cn(
              "flex items-center gap-3 rounded-xl px-4 py-3 sm:px-5 sm:py-4 text-left font-bold text-white transition-all duration-300",
              "animate-answer-reveal backdrop-blur-sm",
          ...
          >
            {/* Letter badge */}
            <span
              className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-lg text-xs sm:text-sm font-black flex-shrink-0"
