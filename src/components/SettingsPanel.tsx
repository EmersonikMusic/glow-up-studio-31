...
const EXTRA_ROW_HEIGHT = 42;
const SECTION_CONTENT_MAX = 2000; // large enough for any section
...
function ToggleRow({
  ...
  return (
    ...
    >
      ...
      <span className="text-xs font-black tracking-widest uppercase transition-colors" style={{ color: active ? "hsl(185 70% 70%)" : "hsl(var(--muted-foreground))" }}>
        {label}
      </span>
    </div>
  );
...
export default function SettingsPanel({ open, onClose }: SettingsPanelProps) {
  ...
  // Section open/collapsed state — Categories open by default
  const [catSectionOpen, setCatSectionOpen] = useState(true);
  const [diffSectionOpen, setDiffSectionOpen] = useState(false);
  const [eraSectionOpen, setEraSectionOpen] = useState(false);
  const [gameSettingsOpen, setGameSettingsOpen] = useState(false);

  // Item expand state (within sections)
  ...
  const categoriesVisible = categories.slice(0, 6);
  const categoriesExtra = categories.slice(6);
      ...
      >
        ...
        >
          ...
          <section className="mx-4 mb-3 rounded-2xl flex flex-col overflow-hidden" style={{ background: "hsl(240 42% 15%)", border: "1px solid hsl(var(--game-card-border))" }}>
            <SectionHeader icon={<LayoutGrid className="w-4 h-4 text-[hsl(185_70%_55%)]" />} label="Categories" open={catSectionOpen} onToggle={() => setCatSectionOpen((v) => !v)} />
            <div
              className="flex flex-col overflow-hidden"
              style={{ maxHeight: catSectionOpen ? `${SECTION_CONTENT_MAX}px` : "0px", transition: "max-height 0.4s cubic-bezier(0.16, 1, 0.3, 1)" }}
            >
              {/* All toggle */}
              ...
              {/* Animated extra */}
              <div className="flex flex-col overflow-hidden" style={{ maxHeight: categoriesExpanded ? `${categoriesExtra.length * EXTRA_ROW_HEIGHT}px` : "0px", transition: "max-height 0.4s cubic-bezier(0.16, 1, 0.3, 1)" }}>
                {categoriesExtra.map((cat) => (
                  <ToggleRow key={cat} label={cat} active={selectedCategories.includes(cat)} onClick={() => toggleCategory(cat)} />
                ))}
              </div>
              <button onClick={() => setCategoriesExpanded((v) => !v)} className="flex items-center justify-center py-3 w-full transition-colors hover:bg-[hsl(240_42%_18%)]">
                {categoriesExpanded
          ...
          {/* ── DIFFICULTY ── */}
          <section className="mx-4 mb-3 rounded-2xl flex flex-col overflow-hidden" style={{ background: "hsl(240 42% 15%)", border: "1px solid hsl(var(--game-card-border))" }}>
            <SectionHeader icon={<BarChart2 className="w-4 h-4 text-[hsl(185_70%_55%)]" />} label="Difficulty" open={diffSectionOpen} onToggle={() => setDiffSectionOpen((v) => !v)} />
            <div
              className="flex flex-col overflow-hidden"
              style={{ maxHeight: diffSectionOpen ? `${SECTION_CONTENT_MAX}px` : "0px", transition: "max-height 0.4s cubic-bezier(0.16, 1, 0.3, 1)" }}
            >
              <ToggleRow label="All Difficulties" active={allDiffsSelected} onClick={toggleAllDifficulties} />
              {difficulties.map((diff) => (
                <ToggleRow key={diff} label={diff} active={selectedDifficulties.includes(diff)} onClick={() => toggleDifficulty(diff)} />
              ))}
            </div>
          </section>
          ...
          {/* ── ERAS ── */}
          <section className="mx-4 mb-3 rounded-2xl flex flex-col overflow-hidden" style={{ background: "hsl(240 42% 15%)", border: "1px solid hsl(var(--game-card-border))" }}>
            <SectionHeader icon={<Hourglass className="w-4 h-4 text-[hsl(185_70%_55%)]" />} label="Eras" open={eraSectionOpen} onToggle={() => setEraSectionOpen((v) => !v)} />
            <div
              className="flex flex-col overflow-hidden"
              style={{ maxHeight: eraSectionOpen ? `${SECTION_CONTENT_MAX}px` : "0px", transition: "max-height 0.4s cubic-bezier(0.16, 1, 0.3, 1)" }}
            >
              ...
              <button onClick={() => setErasExpanded((v) => !v)} className="flex items-center justify-center py-3 w-full transition-colors hover:bg-[hsl(240_42%_18%)]">
                {erasExpanded
                  ? <span className="text-[10px] font-black tracking-widest text-[hsl(185_70%_55%)] uppercase">Show less ↑</span>
                  : <MoreHorizontal className="w-5 h-5 text-[hsl(185_70%_55%)]" />}
              </button>
          ...
          <section className="mx-4 mb-6 rounded-2xl flex flex-col overflow-hidden" style={{ background: "hsl(240 42% 15%)", border: "1px solid hsl(var(--game-card-border))" }}>
            <SectionHeader icon={<SlidersHorizontal className="w-4 h-4 text-[hsl(185_70%_55%)]" />} label="Game Settings" open={gameSettingsOpen} onToggle={() => setGameSettingsOpen((v) => !v)} />
            <div
              className="overflow-hidden"
              style={{ maxHeight: gameSettingsOpen ? `${SECTION_CONTENT_MAX}px` : "0px", transition: "max-height 0.4s cubic-bezier(0.16, 1, 0.3, 1)" }}
            >
