              ...
              <div className="px-5 py-5 flex flex-col gap-6">
                ...
                <div>
                  ...
                  <div className="step-slider-wrap">
                    <input type="range" min={10} max={50} step={10} value={numQuestions} onChange={(e) => setNumQuestions(Number(e.target.value))} className="step-slider w-full" />
                    <div className="absolute inset-x-0 flex justify-between pointer-events-none" style={{ top: "50%", transform: "translateY(-50%)", zIndex: 1, padding: "0 11px" }}>
                      {[10,20,30,40,50].map((v) => (
                        <div key={v} className="rounded-full transition-colors duration-200 flex-shrink-0" style={{ width: 5, height: 5, background: "#fff", opacity: numQuestions >= v ? 1 : 0.35 }} />
                      ))}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="step-slider-wrap">
                    ...
                    <div className="absolute inset-x-0 flex justify-between pointer-events-none" style={{ top: "50%", transform: "translateY(-50%)", zIndex: 1, padding: "0 11px" }}>
                      {[5,10,15,20,25,30].map((v) => (
                        <div key={v} className="rounded-full transition-colors duration-200 flex-shrink-0" style={{ width: 5, height: 5, background: "#fff", opacity: timePerQuestion >= v ? 1 : 0.35 }} />
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="step-slider-wrap">
                    <input type="range" min={5} max={30} step={5} value={timePerAnswer} onChange={(e) => setTimePerAnswer(Number(e.target.value))} className="step-slider w-full" />
                    <div className="absolute inset-x-0 flex justify-between pointer-events-none" style={{ top: "50%", transform: "translateY(-50%)", zIndex: 1, padding: "0 11px" }}>
                      {[5,10,15,20,25,30].map((v) => (
                        <div key={v} className="rounded-full transition-colors duration-200 flex-shrink-0" style={{ width: 5, height: 5, background: "#fff", opacity: timePerAnswer >= v ? 1 : 0.35 }} />
                      ))}
                    </div>
                  </div>
                </div>
